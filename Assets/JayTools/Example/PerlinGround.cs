    ...
    public void Randomize()
    {
        ...
        for (int i = 0; i < Size.x; i++)
        {
            for (int j = 0; j < Size.y; j++)
            {
                ...
                float sample = (float)perlin.perlin(initialOffsetColor + ((i * 1f)/ Size.x), initialOffsetColor + ((j * 1f)/ Size.y), 0f);
                cube.GetComponent<Renderer>().material.color = leftColor + sample * (rightColor - leftColor);

                sample = (float)perlin.perlin(initialOffsetHeight + ((i * 1f)/ Size.x), initialOffsetHeight + ((j * 1f)/ Size.y), 0f);
